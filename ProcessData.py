...
def process_dataset(root_dir, output_file='processed_data.pkl'):
    ...
    with open(output_file, 'wb') as f:
        pickle.dump({'MoveData': MoveData, 'MoveType': MoveType}, f)
    ...
    print("\n--- Weryfikacja danych ---")
    print(f"MoveData shape: {MoveData.shape}")  # (liczba_okien, WINDOW_SIZE, liczba_cech)
    print(f"MoveType shape: {MoveType.shape}")  # (liczba_okien,)

    unique_labels = np.unique(MoveType)
